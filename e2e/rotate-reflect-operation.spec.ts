...
const EXPECTED_ROTATION_LABELS = [
  "Rotate 90 clockwise",
  "Rotate 180",
  "Rotate 270 clockwise",
];

const EXPECTED_REFLECTION_LABELS = ["Flip horizontal", "Flip vertical"];
...
test("Rotate offers only the rotation choices and Reflect offers only the reflection choices", async () => {
  ...
  await openGeometricTransformFromMenu(launched.app, launched.window, "flip-horizontal");
  expect(await readOfferedGeometricTransformLabels(launched.window, "flip-horizontal")).toEqual(EXPECTED_REFLECTION_LABELS);
});

test("Rotate 90 swaps width/height and lands a pixel at (H-1-y, x)", async () => {
  await loadNonSquareStackIntoPanelOne();
