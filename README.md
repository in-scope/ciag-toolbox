...
- Step through the bands of a multi-band image with the **band control** at the bottom of the cell: drag the slider, type a band number, use the arrow keys, or scroll the mouse wheel over the control. The control shows the current band and the total, and scales smoothly to images with hundreds of bands. Single-band images do not show it.
