...
// two; callers pad (see spatial-frequency-filter.ts).
...
export function inverseFftInPlace(real: Float64Array, imag: Float64Array): void {
...
type ComplexLineTransform = (real: Float64Array, imag: Float64Array) => void;
...
function transformEveryGridColumn(grid: ComplexGrid, transformLine: ComplexLineTransform): void {
  const columnReal = new Float64Array(grid.height);
  const columnImag = new Float64Array(grid.height);
...
function copyGridColumnIntoLine(
  grid: ComplexGrid,
  x: number,
  lineReal: Float64Array,
  lineImag: Float64Array,
): void {
  for (let y = 0; y < grid.height; y += 1) {
...
function transformInPlace(real: Float64Array, imag: Float64Array, sign: number): void {
...
function assertTransformBuffersAreUsable(real: Float64Array, imag: Float64Array): void {
...
function permuteIntoBitReversedOrder(real: Float64Array, imag: Float64Array): void {
...
function swapComplexEntries(real: Float64Array, imag: Float64Array, a: number, b: number): void {
  swapValuesAt(real, a, b);
  swapValuesAt(imag, a, b);
}

function swapValuesAt(values: Float64Array, a: number, b: number): void {
  const held = values[a] ?? 0;
  values[a] = values[b] ?? 0;
  values[b] = held;
}

function scaleValuesInPlace(values: Float64Array, factor: number): void {
